...
<p7:protocol xmlns:p7="http://wired.read-write.fr/P7/Specification"
  ...
  <p7:fields>
    ...
    <!-- Per-session client nonce; server uses it to derive the expected salted password hash -->
    <p7:field name="p7.encryption.password_salt" id="17" type="data" />
    <!-- Per-user stored salt sent encrypted by the server; client uses it to derive the base hash -->
    <p7:field name="p7.encryption.stored_salt"   id="18" type="data" />
    <!-- New in P7 v1.3: server persistent identity key (raw P256 public key, 64 bytes) -->
    <p7:field name="p7.encryption.server_identity_key" id="19" type="data" />
    <!-- New in P7 v1.3: ECDSA signature over the ephemeral ECDH public key, signed by identity key -->
  ...
  <p7:messages>
    ...
    <!-- P7 v1.3: server_key now carries the persistent identity key and its signature for TOFU -->
    <p7:message name="p7.encryption.server_key" id="4">
      <p7:parameter field="p7.encryption.public_key" use="required" />
      <p7:parameter field="p7.encryption.server_identity_key" />
      <p7:parameter field="p7.encryption.server_identity_sig" />
      <p7:parameter field="p7.encryption.strict_identity" />
    </p7:message>

    <!-- In P7 v1.2 cipher.key and username moved to username_request (separate round trip) -->
    <p7:message name="p7.encryption.client_key" id="5">
      <p7:parameter field="p7.encryption.client_password" use="required" />
      <p7:parameter field="p7.encryption.password_salt"   use="required" />
    </p7:message>
    ...
    <!-- New in P7 v1.2: client sends identity first so server can look up per-user salt -->
    <p7:message name="p7.encryption.username_request" id="10">
  ...
  <p7:transactions>
    ...
    <p7:transaction message="p7.encryption.server_key" originator="server" use="required">
      <p7:reply message="p7.encryption.username_request" count="1" use="required" />
    </p7:transaction>

    <p7:transaction message="p7.encryption.username_request" originator="client" use="required">
      <p7:reply message="p7.encryption.server_challenge" count="1" use="required" />
    </p7:transaction>

    <p7:transaction message="p7.encryption.server_challenge" originator="server" use="required">
