...
public class FilesController {
    ...
    let subscriptionsQueue = DispatchQueue(label: "wired3.files.subscriptions")
    var subscribedRealPathsByClient: [UInt32: Set<String>] = [:]
    var subscribedVirtualPathsByClient: [UInt32: [String: Set<String>]] = [:]
    ...
    func isManagedDirectoryType(_ type: File.FileType?) -> Bool {
        guard let type else { return false }
        return type == .dropbox || type == .sync
    }

    func executableFlag(forRealPath path: String) -> Bool {
    ...
    func isDirectoryType(_ type: File.FileType?) -> Bool {
        guard let type else { return false }
